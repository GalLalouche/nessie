    ...
    case ActionTaken(a) =>
      ...
      a match {
        case MovementAction(m@Movement(src, dst)) =>
          ...
          val o = gs.map(src)
          val movedMap = gs.&|->(GameState.mapAndFogs).^|->(MapAndFogs.map).modify(_.remove(src) |> (_.place(dst, o)))
          val updatedFow = movedMap
              .&|->(GameState.mapAndFogs)
              .^|->(MapAndFogs.fogForOwner(o.asInstanceOf[CombatUnitObject].unit.owner))
              .modify(_.updateVisible(
                // TODO replace with injection
                TeamFov.visibleForOwner(movedMap.map(dst).asInstanceOf[CombatUnitObject], movedMap.map))
              )
          updatedFow.&|->(GameState.currentTurn).modify(_.map(_ + MovementWithDistance(m, distance)))
